...
-record(state, {reference_datetime :: calendar:datetime(),
                datetime_at_reference :: diablo_cron:seconds()}).
...
init([]) ->
    ...
    {ok, #state{reference_datetime=DateTime,
                datetime_at_reference=?utils:epoch_seconds()}}.
...
handle_call(get_datetime, _From, State = #state{reference_datetime = DateTime,
    ...
    %% ?DEBUG("get_datetime: datetime ~p, actual ~p", [DateTime, Actual]),
    {reply, {DateTime, Actual}, State};
handle_call({set_datetime, DateTime}, _From, State) ->
    ?DEBUG("set_datetime ~p", [DateTime]),
...
internal_cancel(AlarmRef) ->
    case ?cron_job_regist:get(AlarmRef) of
        undefined ->
            undefined;
        {ok, [Pid]} ->
            ?cron_agent:cancel(Pid)
    end.
