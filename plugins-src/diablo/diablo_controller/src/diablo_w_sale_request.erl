...
action(Session, Req, {"delete_daily_cost"}, Payload) ->
    ?DEBUG("update_daily_cost: session ~p, payload ~p", [Session, Payload]),
    Merchant = ?session:get(merchant, Session),
    CostId = ?v(<<"cid">>, Payload),
    ?utils:respond(
       normal,
       fun() -> ?shop:cost(delete, Merchant, CostId) end,
       fun(_CostId) -> ?succ(add_shop, _CostId) end,
       Req); 
...
action(Session, Req, {"check_w_pay_scan"}, Payload) ->
    ...
    PayUse   = ?v(<<"use">>, Payload),
    PayTime   = ?utils:format_date(yyyyMMdd, ?utils:to_date(datetime, ?v(<<"pay_time">>, Payload))),
    case ?shop:shop(get, Merchant, ShopId) of
	{ok, Shop} ->
...
start_pay(0, Merchant, Shop, PayType, MchntCd, PayCode, Balance) ->
	...
	{ok, ?PAY_SCAN_SUCCESS, PayOrder, RealPayType, PayBalance} ->
	    ...
	    case ?w_sale:pay_scan(
		    ...
		    Merchant,
		    ShopId,
		    {RealPayType, ?PAY_SCAN_SUCCESS, ?NEW_SALE, PayOrder, PayBalance}) of
		{ok, _} ->
	...
	{error, ?PAY_SCAN_UNKOWN, PayOrderNo} ->
	    ...
	    {?err(pay_scan_unkown, PayOrderNo), [{<<"pay_order">>, ?to_b(PayOrderNo)}]}; 
...
start_pay(1, Merchant, Shop, PayType, MchntCd, PayCode, Balance) ->
	...
	{ok, ?PAY_SCAN_UNKOWN, PayOrder, _RealPayType} ->
	    ?w_sale:pay_scan(
	       start,
	       Merchant,
	       ShopId,
	       {PayType, ?PAY_SCAN_UNKOWN, ?NEW_SALE, PayOrder, Balance, PayTime}),
	...
	false ->
		...
		{ok, _} ->
			...
			{error, PayState, RealFee} ->
				...
				?PAY_SCAN_FAILED -> 
				    ?err(pay_scan_failed, PayOrder);
				?PAY_SCAN_PAYING -> 
				    ?err(pay_scan_paying, PayOrder);
				?PAY_SCAN_CLOSED ->
				    ?err(pay_scan_closed, PayOrder)
			    end;
...
check_pay(2, Merchant, Shop, MchntCd, PayOrder, PayTime) ->
	...
	{ok, PayState, PayType, PayBalance} ->
		...
		{ok, _} -> 
			...
			?PAY_SCAN_SUCCESS ->
			    ...
			    %%  [{<<"balance">>, PayBalance},
			    %%   {<<"pay_type">>, PayType},
			    %%   {<<"pay_state">>, ?PAY_SCAN_SUCCESS}]};
			    {?succ(check_pay_scan, PayOrder),
			     Extra ++ [{<<"pay_state">>, ?PAY_SCAN_SUCCESS}]};
			?PAY_SCAN_FAILED ->
			    ?err(pay_scan_failed, PayOrder);
			?PAY_SCAN_UNKOWN ->
			    ?err(pay_scan_unkown, PayOrder);
		...
		{error, _CheckError} ->
		    {?err(check_pay_scan_but_db_error, PayOrder), Extra}
	    end;
	{error, ?PAY_SCAN_ABNORMAL, PayReturnCode, Msg} ->
	    {?err(pay_scan_abnormal, PayOrder),
	     [{<<"pay_code">>, PayReturnCode}, {<<"msg">>, ?to_b(Msg)}]}; 
	{error, pay_http_trans_failed, Reason} ->
	    ?err(check_pay_http_trans_failed, Reason);
	{error, pay_http_failed, HttpCode} ->
	    ?err(check_pay_http_failed, HttpCode) 
    end.
