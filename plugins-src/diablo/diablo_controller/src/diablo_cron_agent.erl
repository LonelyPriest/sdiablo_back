...
-spec start_link/2::(diablo_cron:job_ref(), diablo_cron:job()) ->
...
-spec set_datetime/3 :: (pid(), calendar:datetime(), ?cron:seconds()) -> ok. 
...
validate(Spec) ->
    State = #state{job=undefined,
                   alarm_ref=undefined},
    {DateTime, Actual} = ?cron_control:datetime(),
...
init([JobRef, Job]) ->
    ...
    case until_next_milliseconds(NewState, Job) of
        {ok, Millis} when is_integer(Millis) ->
            ?cron_job_regist:register(JobRef, self()),
            {ok, NewState, Millis};
        {error, _}  ->
            {stop, normal}
    end.
...
handle_cast(shutdown, State) ->
    %% ?DEBUG("shutdown ~p", [State]),
    {stop, normal, State};
handle_cast({set_datetime, DateTime, Actual}, State) ->
    ?DEBUG("set_datetime datetime ~p, actual ~p", [DateTime, Actual]),
...
terminate(_Reason, #state{alarm_ref=Ref}) ->
    ?DEBUG("termiante with alarm_ref ~p", [Ref] ),
    ?cron_job_regist:unregister(Ref),
    ok.
...
current_date(State) ->
    Elapsed = ?utils:epoch_seconds() - State#state.seconds_at_reference,
    calendar:gregorian_seconds_to_datetime(
      ceiling(Elapsed + State#state.referenced_seconds)).
...
fast_forward(State, NewDate) ->
    io:format("fast forward ~p", [NewDate]),
    try
