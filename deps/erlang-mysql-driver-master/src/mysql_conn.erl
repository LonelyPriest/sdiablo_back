...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
    ...
    ?Log2(LogFun, debug, "fetch ~ts (id ~p)", [Query1,RecvPid]),
...
do_transaction(State, Fun) ->
    %% io:format("~p: do_transaction~n", [?LINE]),
    case do_query(State, <<"BEGIN">>) of
 	...
 	_ ->
		...
		Res ->
		    %% io:format("~p: res ~p~n", [?LINE, Res]),
		    case do_query(State, <<"COMMIT">>) of
			{error, _} = Err ->
			    rollback(State, {commit_error, Err});
			_Any ->
			    %% io:format("~p: any ~p~n", [?LINE, _Any]),
			    case Res of
				{atomic, _} ->
				    %% io:format("~p: return res ~p~n", [?LINE, Res]),
				    Res;
				_ ->
				    %% io:format("~p: return atomic res ~p~n", [?LINE, Res]),
				    {atomic, Res}
			    end
		    end
	    end
...
get_query_response(LogFun, RecvPid, Version) ->
	...
	{ok, Packet, _} ->
		...
		0 ->
		    ...
		    R = {updated, #mysql_result{affectedrows=AffectedRows, insertid=InsertId}},
		    %% io:format("~p: R ~p~n", [?LINE, R]),
		    R;
		255 ->
		    case get_error_data(Rest, Version) of
			{Code, {SqlState, Message}} ->
			    % MYSQL_4_1 error data
			    %% io:format("~p: message ~p~n", [?LINE, Message]),
